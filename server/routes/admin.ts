...
/**
 * Get all career options for admin management
...
router.get("/career-options", requirePermission("read:careers"), async (req, res) => {
  try {
    ...
    if (isActive !== undefined) {
      conditions.push(eq(careerOptions.isActive, isActive === 'true'));
    }

    // Get total count
    const [totalResult] = conditions.length > 0 ? 
      await db.select({ count: count() }).from(careerOptions).where(and(...conditions)) :
      await db.select({ count: count() }).from(careerOptions);
    const total = totalResult.count || 0;
    ...
    const baseQuery = db
      .select({
        ...
        description: careerOptions.description,
        salaryMin: careerOptions.salaryMin,
        salaryMax: careerOptions.salaryMax,
        difficultyLevel: careerOptions.difficultyLevel,
        requiredSkills: careerOptions.requiredSkills,
        growthOutlook: careerOptions.growthOutlook,
        isActive: careerOptions.isActive,
        createdAt: careerOptions.createdAt,
        updatedAt: careerOptions.updatedAt
      })
      .from(careerOptions)
      .orderBy(sql`${careerOptions.updatedAt} DESC`)
      .limit(limitNum)
      .offset(offset);

    const careersList = conditions.length > 0 ? 
      await baseQuery.where(and(...conditions)) : 
      await baseQuery;
...
router.put("/career-options/:id", requirePermission("update:careers"), async (req, res) => {
  try {
    ...
    const [updatedCareer] = await db
      ...
      .where(eq(careerOptions.id, careerId))
      .returning();
...
/**
 * Get career paths for admin management
...
router.get("/career-paths", requirePermission("read:careers"), async (req, res) => {
  try {
    ...
    if (isActive !== undefined) {
      conditions.push(eq(careerPaths.isActive, isActive === 'true'));
    }

    // Get total count
    const [totalResult] = conditions.length > 0 ? 
      await db.select({ count: count() }).from(careerPaths).where(and(...conditions)) :
      await db.select({ count: count() }).from(careerPaths);
    const total = totalResult.count || 0;
    ...
    const baseQuery = db
      .select()
      .from(careerPaths)
      .orderBy(sql`${careerPaths.createdAt} DESC`)
      .limit(limitNum)
      .offset(offset);
 ...
 */
router.post("/career-paths", requirePermission("create:careers"), async (req, res) => {
  try {
    const {
      title,
      category,
