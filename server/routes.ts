...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/users/:id/dashboard", async (req, res) => {
    try {
      ...
      // Get user's quiz results for career path
      const quizResults = await storage.getQuizResultsByUser(userId);
      const latestQuizResult = quizResults[0]; // Most recent quiz result
      ...
      // Get recommended courses and projects
      const recommendedCourses = await storage.getRecommendedCourses(userId);
      const recommendedProjects = await storage.getRecommendedProjects(userId);
      ...
      // Get today's daily byte
      const todaysDailyByte = await storage.getTodaysDailyByte();
      
      // Calculate user's daily byte streak
      const userDailyBytes = await storage.getUserDailyBytesByUser(userId);
      const completedCount = userDailyBytes.filter(udb => udb.completed).length;

      // Calculate progress metrics
      const completedCourses = enrollments.filter(e => e.isCompleted).length;
      const totalEnrolledCourses = enrollments.length;
      const completedProjects = userProjects.filter(up => up.isCompleted).length;
      const totalProjects = userProjects.length;
      const completedSoftSkills = userSoftSkills.filter(uss => uss.isCompleted).length;
      
      // Calculate overall progress percentage
      const hasCareerAssessment = quizResults.length > 0;
      const hasNicheSelection = latestQuizResult?.recommendedNiches?.length > 0;
      
      let progressPercentage = 0;
      if (hasCareerAssessment) progressPercentage += 25;
      if (hasNicheSelection) progressPercentage += 25;
      if (completedCourses > 0) progressPercentage += 25;
      if (completedProjects > 0) progressPercentage += 25;
      ...
      const dashboardData = {
        ...
        progress: {
          ...
          careerAssessment: hasCareerAssessment,
          nicheSelection: hasNicheSelection,
          coreCourses: {
            completed: completedCourses,
            total: Math.max(totalEnrolledCourses, 3)
          },
          projects: {
            completed: completedProjects,
            total: Math.max(totalProjects, 2)
          }
        },
        careerPath: latestQuizResult ? {
          title: latestQuizResult.recommendedCareer,
          niches: latestQuizResult.recommendedNiches,
          match: 85 // Could be calculated from quiz results
        } : null,
        achievements: userAchievements.slice(0, 3), // Show top 3 recent achievements
        recommendedCourses: recommendedCourses.slice(0, 6),
        recommendedProject: recommendedProjects[0] || null,
        upcomingEvents: upcomingEvents.slice(0, 3),
        communityPosts: recentPosts,
        dailyByte: todaysDailyByte,
        dailyByteStreak: completedCount
      };
